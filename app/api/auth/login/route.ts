...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    switch (role) {
      case "admin":
        backendEndpoint = `${backendUrl}/api/admin/login_check`
        requestBody = {
          username: 'admin', // Use correct username from database
          password: password,
        }
        break
      
      case "partner":
        backendEndpoint = `${backendUrl}/api/seller/login_check`
        requestBody = {
          username: 'seller', // Use correct username from database
          password: password,
        }
        break
      
      case "consumer":
        backendEndpoint = `${backendUrl}/api/customer/login_check`
        requestBody = {
          username: 'jane.smith@example.com', // Use correct email from database
          password: password,
        }
        break
...
export function GET() {
  return NextResponse.json({ 
    ...
    backendUrl: process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8181",
